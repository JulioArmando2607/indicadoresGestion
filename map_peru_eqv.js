// Esta función debería llamarse solo una vez cuando la página se carga.
...
function cargarData(valor) {
    ...
    fetch(env+'public/seguimientoequipo/listarEquiposInformaticosIndicadorMapa', {
...
async function cargarTambos(id) {
    console.log(id)
    try {
        const response = await fetch(`${env}public/seguimientoequipo/listarMapaEquiposRegionTambos/${id}`);
...
async function cargarTambosDatosv(valor) {
    ...
    try {
        ...
        let response = await fetch(env+'public/seguimientoequipo/listarEquiposInformaticosIndicadorMapa', {
            method: 'POST',
            headers: {
